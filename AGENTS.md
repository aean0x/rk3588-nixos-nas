...
- **Models**: `hermesPnP.models` low/medium/high (deepseek flash / pro / xai-oauth grok-4.6). model-router v0.8.0: Auto classifies all three (Quick / Standard / Expert); `high` is money / irreversible / security. Fallback is deepseek-v4-pro. Do not set `model.context_length`. One-time `hermes auth add xai-oauth` after deploy (`hosts/system/hermes/BOOTSTRAP.md`).
