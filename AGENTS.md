...
- **Models**: `hermesPnP.models` low/medium/high (deepseek flash / pro / xai-oauth grok-4.6). model-router v0.8.1: Auto classifies all three (Quick / Standard / Expert); `high` is money / irreversible / security. Slot IDs come from this declaration (plugin Python has none). Fallback is deepseek-v4-pro. Do not set `model.context_length`. One-time `hermes auth add xai-oauth` after deploy (`hosts/system/hermes/BOOTSTRAP.md`).
