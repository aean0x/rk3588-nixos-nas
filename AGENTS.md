...
| BTA (Minecraft Beta 1.7.3 fork) | Native | `services/bta-server.nix` | `minecraft.<domain>:25565` LAN/Tailscale (no Caddy, no tunnel); 1 GiB RAM + 2 GiB swap, OOM +100 |
...
- **OOM (8 GiB):** Hermes is tertiary vs AdGuard + HA. Agent **2 GiB**, WebUI **3 GiB**, browser **1 GiB**, gbrain **512 MiB** (`runtime.nix`); BTA **1 GiB RAM + 2 GiB swap** (world outranks the agent; `bta-server.nix`); host **8 GiB** swap (`partitions.nix`). Heavy nix eval/build → workstation.
