...
- **Models**: `hermesPnP.models` low/medium/high (deepseek flash / pro / xai-oauth grok-4.6). `hermesPnP.model.default = "high"` (library default is `medium`). model-router **v0.8.5**: Auto classifies all three (Quick / Standard / Expert), sticky prev-tier (high never sticks), compact-on-switch; `high` is money over $20 / irreversible / security. Slot model/provider/label/short/best_for are Nix options. Fallback is deepseek-v4-pro. Do not set `model.context_length`. One-time `hermes auth add xai-oauth` after deploy (`hosts/system/hermes/BOOTSTRAP.md`).
...
- **Edge**: WebUI `archimedes.${domain}:8787` (Caddy LAN + Cloudflare Tunnel). App/gateway alias `hermes.${domain}` → `hermes serve` `:9119` (LAN/Tailscale only, no tunnel, no dashboard UI). Desktop session token is `$HERMES_HOME/desktop-session.token` (runtime, not sops). Browser gate `browser.${domain}:4848` (LAN/Tailscale only).
