...
- **Models**: `hermesPnP.models` low/default/high (deepseek flash / pro / xai-oauth grok-4.6). `hermesPnP.model.default = "high"` (library default is the `default` slot, ex-medium). model-router **v0.8.5**: Auto classifies all three (Quick / Standard / Expert), sticky prev-tier (high never sticks), compact-on-switch; `high` is money over $20 / irreversible / security. Slot model/provider/label/short/best_for are Nix options. Fallback is deepseek-v4-pro. Do not set `model.context_length`. Do not set `hermesPnP.desktop.enable` while the agent jail is on. One-time `hermes auth add xai-oauth` after deploy (`hosts/system/hermes/BOOTSTRAP.md`).
