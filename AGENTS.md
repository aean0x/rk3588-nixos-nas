...
- **Models**: `hermesPnP.modelRouter.enable = false`. Primary is `models.default` (`deepseek` / `deepseek-flash`) which seeds session, cron, and delegation. Official `settings.fallback_model` is `xai-oauth` / `grok-4.6` (PnP would copy `models.default` with the router off). Auxiliary is flash. Do not set `model.context_length`. Do not set `hermesPnP.desktop.enable` while the agent jail is on. One-time `hermes auth add xai-oauth` after deploy (`hosts/system/hermes/BOOTSTRAP.md`) so grok fallback works.
