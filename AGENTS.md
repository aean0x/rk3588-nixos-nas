...
- `kernelPackage` — Kernel package attr (currently `linuxPackages_7_2`)
...
- Local browser: `services.hermesPnP.browser` (declared in `hosts/system/hermes/default.nix`, Brave engine) — sticky profile `/var/lib/hermes/browser-profile`, CDP `127.0.0.1:9222`, **browser-ui gate on :4848** (`https://browser.<domain>/`, LAN/Tailscale, no Cloudflare tunnel). Primary for checkout; Browserless is secondary scraping only.
...
| BTA (Minecraft Beta 1.7.3 fork) | Native | `services/bta-server.nix` | `minecraft.<domain>:25565` LAN/Tailscale (no Caddy, no tunnel); MSH sleep proxy, Java off until join; 1 GiB RAM + 2 GiB swap, OOM +100 |
...
- **Models**: `hermesPnP.modelPicker.enable = false`. Primary is `models.default` (`deepseek` / `deepseek-flash`) which seeds session, cron, and delegation. Official `settings.fallback_model` is `xai-oauth` / `grok-4.6` (PnP would copy `models.default` with the picker off). Auxiliary is flash. Do not set `model.context_length`. Do not set `hermesPnP.desktop.enable` while the agent jail is on. One-time `hermes auth add xai-oauth` after deploy (`hosts/system/hermes/BOOTSTRAP.md`) so grok fallback works.
...
- **OOM (8 GiB):** Hermes is tertiary vs AdGuard + HA. Agent **2 GiB**, WebUI **3 GiB**, browser **1 GiB**, gbrain **512 MiB** (`runtime.nix`); BTA **1 GiB RAM + 2 GiB swap** while awake (world outranks the agent; `bta-server.nix`), MSH a few MB while asleep; host **8 GiB** swap (`partitions.nix`). Heavy nix eval/build → workstation.
...
- ISO/netboot stay true-cross (`localSystem` + `crossSystem`). Live `remote-*` userspace is qemu-native aarch64 (binfmt required) — gh/haskell/dotnet/zig cannot cross. Kernel is still `pkgsCross` `CROSS_COMPILE` so gcc is not qemu-user.
...
- Kernel 7.2 for rk3588 is a real `CROSS_COMPILE` (x86_64-hosted aarch64 gcc), not qemu. Version bumps rebuild from source because `structuredExtraConfig` misses Hydra cache. The kernel drv ignores deploy `--cores` and uses `nproc`.
