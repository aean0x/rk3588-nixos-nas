...
let
  ...
  # Per-agent AGENTS.md protected sections (shared base + tool summary + optional blurb)
  ...
  subAgentAgentsProtectedFor =
    ...
    let
      ...
      toolSummary = agentDefs.mkToolSummary id;
      blurb = if ovr.agentsMdBlurb != null then "\n${ovr.agentsMdBlurb}" else "";
    in
    pkgs.writeText "${id}-agents-protected" (baseProtected + "\n" + toolSummary + blurb);
