...
- Models: `hermesPnP.models` low/medium/high (deepseek-v4-flash / pro / xai-oauth grok-4.6). Split is only those three. model-router **v0.7.0**: Auto classifies low vs medium; `high` is `escalate_model` or `/high`. Fallback is deepseek-v4-pro. Do not set `model.context_length` (stamps every model); grok cliff is `compression.threshold_tokens`. Do not pin `agent.max_turns`.
