...
- Models: `hermesPnP.models` low/default/high (deepseek-v4-flash / pro / xai-oauth grok-4.6). Split is only those three. `hermesPnP.model.default = "high"` (library default is the `default` slot, ex-medium). model-router **v0.8.5**: Auto classifies all three (Quick / Standard / Expert), sticky prev-tier (high never sticks), compact-on-switch; classifier LLM call is capped at 8s (`classifier_timeout_s`) so it cannot blow the 30s `pre_llm_call` budget; `high` is money over $20 / irreversible / security. Slot model/provider/label/short/best_for are Nix options (generated config.json is the Python handoff). Fallback is deepseek-v4-pro (consumer override; composer would seed models.high). Auxiliary: `free_only = true` plus `:free` OpenRouter SKU so background tasks never fall onto a paid lane. Do not set `model.context_length` (stamps every model); grok cliff is `compression.threshold_tokens = 200000` (also holds DeepSeek at 200k). Do not pin `agent.max_turns`. Do not set `hermesPnP.desktop.enable` while the agent jail is on — Desktop remote is the host `hermes-serve` unit.
