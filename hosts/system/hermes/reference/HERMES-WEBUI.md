...
- **Model Router extension:** `HERMES_WEBUI_EXTENSION_DIR` → flake `plugins/model-router/webui` (store path). Injects `/auto` `/t1` `/t2` `/t3` composer buttons and overlays the model chip (`Auto · T1 · deepseek-v4-flash`). No WebUI source patches.
