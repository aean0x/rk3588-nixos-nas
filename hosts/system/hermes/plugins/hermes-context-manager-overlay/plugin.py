...
class HermesContextManagerPlugin:
    ...
    def register(self, ctx) -> None:
        ...
        # Observational post_tool_call cannot mutate live history (Hermes fires
        # it before the tool result is appended). Real savings require:
        # - transform_tool_result: rewrite the string entering history
        # - pre_api_request: measure approx_input_tokens every mid-loop call
        #   and prune live tool message content in-place when over budget
    ...
    # ------------------------------------------------------------------
    # Live mutation hooks (2026-07-31)
    # ------------------------------------------------------------------
    ...
    def on_transform_tool_result(
        ...
        **kwargs,
        ...
        try:
            ...
            if not trunc.enabled:
                return None
            min_len = int(getattr(trunc, "min_content_length", 500) or 500)
            if len(result) < max(min_len, 2500):
                return None

            max_lines = int(getattr(trunc, "max_lines", 40) or 40)
            head_n = int(getattr(trunc, "head_lines", 12) or 12)
            tail_n = int(getattr(trunc, "tail_lines", 8) or 8)
            lines = result.splitlines()
            if len(lines) <= max_lines and len(result) < 8000:
                return None
